...
# webos_enactjs_app
#
# This class override the
# /meta-webosose/meta-webos/classes/webos_enactjs_app.bbclass to split the
# packaging of the EnactJS app from the installation of the app due to the
# mksnapshot issue with Pseudo.
# See details at the http://jira.lge.com/issue/browse/NEVA-10931
#
...
PACKAGED_APP_DIR = "${WORKDIR}/packaged_app"
do_create_snapshot() {
    working=$(pwd)
    ...
    # Stage app
    appdir="${PACKAGED_APP_DIR}${webos_applicationsdir}/${WEBOS_ENACTJS_APP_ID}"
...
do_create_snapshot[cleandirs] = "${PACKAGED_APP_DIR}"

do_install() {
    working=$(pwd)
    appdir="${D}${webos_applicationsdir}/${WEBOS_ENACTJS_APP_ID}"

    cp -R "${PACKAGED_APP_DIR}/." "${D}"
